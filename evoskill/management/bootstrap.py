...
def _deterministic_import_id(*, scope: str, owner: str, rel_path: str) -> str:
    ...
    base = f"evoskill-skill-id-v1:{scope}:{owner}:{rel_path}"
...
def run_service_startup_maintenance(
    *,
    sdk: Any,
    default_user_id: str,
    log_prefix: str = "[evoskill][startup]",
) -> Dict[str, Any]:
    """
    Runs startup offline checks/imports for local store.

    Controls:
    - EVOSKILL_AUTO_NORMALIZE_IDS (default: 1)
    - EVOSKILL_AUTO_IMPORT_DIRS (comma-separated paths)
    - EVOSKILL_AUTO_IMPORT_SCOPE (common|user, default: common)
    - EVOSKILL_AUTO_IMPORT_LIBRARY (for common scope)
    - EVOSKILL_AUTO_IMPORT_OVERWRITE (default: 0)
    - EVOSKILL_AUTO_IMPORT_INCLUDE_FILES (default: 1)
    - EVOSKILL_AUTO_IMPORT_MAX_DEPTH (default: 6)
    - EVOSKILL_AUTO_REFRESH_SKILLS (default: 1)
    - EVOSKILL_AUTO_REFRESH_FORCE_VECTORS (default: 0)
    - EVOSKILL_AUTO_REFRESH_BLOCKING (default: 1)
    """
    ...
    try:
        ...
        normalize_enabled = _to_bool(os.getenv("EVOSKILL_AUTO_NORMALIZE_IDS"), default=True)
        max_depth = _to_int(os.getenv("EVOSKILL_AUTO_IMPORT_MAX_DEPTH"), _to_int(cfg.get("auto_import_max_depth"), 6))
        ...
        env_dirs = _to_str_list(os.getenv("EVOSKILL_AUTO_IMPORT_DIRS"))
        auto_import_dirs = []
        seen = set()
        ...
        if auto_import_dirs:
            scope = str(
                os.getenv("EVOSKILL_AUTO_IMPORT_SCOPE")
        ...
        refresh_enabled = _to_bool(os.getenv("EVOSKILL_AUTO_REFRESH_SKILLS"), default=True)
        refresh_force_vectors = _to_bool(
            os.getenv("EVOSKILL_AUTO_REFRESH_FORCE_VECTORS"),
            default=False,
        )
        refresh_blocking = _to_bool(
            os.getenv("EVOSKILL_AUTO_REFRESH_BLOCKING"),
            default=True,
        )
        if refresh_enabled:
