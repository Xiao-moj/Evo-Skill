...
class SkillCandidate:
    ...
    files: Dict[str, Any] = None  # optional extra files e.g. {"scripts/quicksort.py": "..."}
    metadata: Dict[str, Any] = None  # optional extra metadata e.g. {"capabilities": [...]}
...
def build_default_extractor(config: EvoSkillConfig) -> SkillExtractor:
...
class HeuristicSkillExtractor:
    def __init__(self, config: EvoSkillConfig) -> None:
        """Run init."""
        self._config = config
...
class LLMSkillExtractor:
    ...
    def extract(
        ...
        *,
        ...
        system = (
            "You are EvoSkill's Skill Extractor.\n"
            ...
            "User-reuse-first: prefer extraction only when the skill is likely to be reused by this same user in future similar tasks.\n"
            ...
            "\n"
            "### 3) WHEN TO EXTRACT\n"
            "- DO NOT EXTRACT (return {\"skills\": []}) for one-shot/generic tasks with no user policy constraints, trivial common-knowledge logic, assistant-only requirements, stale requirements not reflected in recent turns, or low expected repeat-use value for this user.\n"
            ...
            "- Future reuse test (user-specific): after de-identification, ask whether this user is likely to need the same policy/workflow again in nearby future interactions; if unlikely, return {\"skills\": []}.\n"
            ...
            "- Is this likely to be reused by this same user in future similar tasks (after removing one-off context)?\n"
