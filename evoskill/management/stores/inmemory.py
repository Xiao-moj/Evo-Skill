...
class InMemorySkillStore(SkillStore):
    ...
    def upsert(self, skill: Skill, *, raw: Optional[Dict[str, Any]] = None) -> None:
        ...
        with self._lock:
            ...
            if uid and sid and skill_memory_kind(skill) == MEMORY_KIND_SKILL:
    ...
    def find_user_skills_by_identity_desc_norm(
        ...
        *,
        ...
        """Finds user memories by normalized description identity."""
        ...
        with self._lock:
            for rec in self._records.values():
                skill = rec.skill
                if str(getattr(skill, "user_id", "") or "").strip() != uid:
                    continue
                if identity_desc_norm_from_fields(
                    description=str(getattr(skill, "description", "") or ""),
                    name=str(getattr(skill, "name", "") or ""),
                ) != key:
                    continue
