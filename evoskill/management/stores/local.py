...
from ...memory import MEMORY_KIND_SKILL, skill_memory_kind
...
def _passes_filters(skill: Skill, filters: Dict[str, Any]) -> bool:
    ...
    if isinstance(want_meta, dict):
        for k, v in want_meta.items():
            if str(k) == "memory_kind":
                if skill_memory_kind(skill) != str(v):
                    return False
                continue
...
class LocalSkillStore(SkillStore):
    ...
    def upsert(self, skill: Skill, *, raw: Optional[Dict[str, Any]] = None) -> None:
        ...
        with self._lock:
            ...
            if skill_memory_kind(skill) == MEMORY_KIND_SKILL:
    ...
    def get_with_files(self, skill_id: str) -> Optional[Skill]:
        """Returns a skill and lazily loads bundled files from its backing directory."""
        ...
        return replace(skill, files=dict(loaded.files or {}))
    ...
    def list(self, *, user_id: str) -> List[Skill]:
        ...
        with self._lock:
            return [
                ...
                and skill_memory_kind(r.skill) == MEMORY_KIND_SKILL
            ]
    ...
    def _load_existing(self) -> None:
        ...
        if self._include_legacy_root and os.path.isdir(self._root_dir):
            ...
            skip = {self._users_dirname, self._libraries_dirname, ".evoskill", "vectors"}
            ...
            if self._bm25_startup_mode == "rebuild":
                if self._bm25_index is not None:
                    try:
                        ...
                        print(
                            "[evoskill][bm25] startup rebuild: "
                            f"docs={stat.get('docs', 0)} built={stat.get('built', 0)}"
                        )
                    except Exception as e:
                        print(f"[evoskill][bm25] startup rebuild failed: {e}")
    ...
    def _ensure_bm25_index_health_locked(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            print(
                "[evoskill][bm25] index unhealthy; auto-rebuilt "
                f"(docs={rebuilt.get('docs', 0)}, built={rebuilt.get('built', 0)}), "
                f"issues={report.get('issues', [])}"
            )
        except Exception as e:
            print(
                "[evoskill][bm25] index unhealthy; auto-rebuild failed "
    ...
    def _maybe_migrate_legacy_vector_cache(self) -> None:
        ...
        Best-effort migration from the legacy cache path:
          store_root/.evoskill/vectors -> store_root/vectors
        ...
        try:
            ...
            legacy_dir = os.path.join(self._root_dir, ".evoskill", "vectors")
