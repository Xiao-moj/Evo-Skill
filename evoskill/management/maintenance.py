...
def _judge_merge_with_llm(
    ...
    *,
    ...
    system = (
        "You are EvoSkill's capability identity judge.\n"
...
class SkillMaintainer:
    def __init__(
        self, config: EvoSkillConfig, store: SkillStore, extractor: SkillExtractor
    ) -> None:
        """
        ...
        self._last_upserted_skill_id_by_user: Dict[Tuple[str, str], str] = {}
    ...
    def _get_last_upserted_skill_id(self, *, user_id: str, memory_kind: str) -> Optional[str]:
        ...
        with self._last_lock:
            sid = self._last_upserted_skill_id_by_user.get((uid, str(memory_kind)))
        return str(sid).strip() if sid and str(sid).strip() else None

    def _record_last_upserted_skill_id(
        self,
        *,
        user_id: str,
        skill_id: str,
        ...
        if not uid or not sid:
            return
        with self._last_lock:
            self._last_upserted_skill_id_by_user[(uid, str(memory_kind))] = sid

    def _find_same_identity_user_skills(
        self, *, user_id: str, desc_norm: str, memory_kind: str, limit: int = 8
    ) -> List[Skill]:
        """
                ...
                if isinstance(found, list):
                    return _sort_skills_by_recency(
                        [
                            ...
                            and skill_memory_kind(s) == memory_kind
                        ]
        ...
        for s in candidates or []:
            ...
            if str(getattr(s, "user_id", "") or "").strip() != uid:
                continue
            if skill_memory_kind(s) != memory_kind:
                continue
    ...
    def apply(
        ...
        *,
        ...
        for cand in candidates:
            skill = self._upsert_candidate(cand, user_id=user_id, metadata=metadata, skip_ids=skip_ids or set())
            if skill is not None:
                out.append(skill)
        return out

    def _upsert_candidate(
        self, cand: SkillCandidate, *, user_id: str, metadata: Optional[Dict], skip_ids: Optional[set] = None
    ) -> Optional[Skill]:
        """
        ...
        memory_kind = candidate_memory_kind(cand)
        previous_id, metadata_clean = self._pop_previous_skill_id(metadata)
        previous_id = previous_id or self._get_last_upserted_skill_id(
            user_id=user_id,
            memory_kind=memory_kind,
        )
        merge_gate_cache: Dict[str, str] = {}
        _skip = set(skip_ids or {})

        def _persist_merged(target: Skill) -> Skill:
            """Run persist merged."""
            if memory_kind == MEMORY_KIND_EXPERIENCE:
                merged = _merge(target, cand)
            else:
                merged = (
            ...
            merged.files = _merge_files(_merge_files(target.files, build_agent_skill_files(merged)), dict(cand.files or {}))
            self._store.upsert(merged, raw=_skill_to_raw(merged))
            self._record_last_upserted_skill_id(
                user_id=user_id,
                skill_id=merged.id,
                memory_kind=memory_kind,
            )
        ...
        def _can_merge_to(target: Skill, score: float, *, threshold: float) -> bool:
            ...
            decision = _should_merge(
                llm=(None if memory_kind == MEMORY_KIND_EXPERIENCE else self._llm),
                existing=target,
                candidate=cand,
            ...
            if same_identity:
                ...
                if previous_skill is not None:
                    ...
                    for s in same_identity:
                        if str(getattr(s, "id", "") or "") == prev_id and prev_id not in _skip:
                            target = s
                            break
                if target is None:
                    for s in same_identity:
                        if str(getattr(s, "id", "") or "") not in _skip:
                            target = s
                            break
        ...
        prev_hits: List = []
        prev_score = 0.0
        if previous_skill is not None and prev_threshold > 0 and str(getattr(previous_skill, "id", "")) not in _skip:
        ...
        similar = self._store.search(
            ...
            filters={"scope": "all", "metadata": {"memory_kind": memory_kind}},
        )
        if _skip:
            similar = [h for h in similar if str(getattr(h.skill, "id", "") or "") not in _skip]
        similar_for_llm = (
        ...
        def _create_new() -> Skill:
            ...
            created = Skill(
                ...
                metadata=_with_identity_metadata(
                    _merge_metadata(
                        {},
                        merge_memory_metadata(metadata_clean, memory_kind=memory_kind),
                        cand,
                    ),
                    name=new_name,
                    description=new_description,
            ...
            created.files = _merge_files(build_agent_skill_files(created), dict(cand.files or {}))
...
def _decide_candidate_action_with_llm(
    ...
    *,
    ...
    system = (
        "You are EvoSkill's Skill Set Manager.\n"
...
def _merge_with_llm(llm, existing: Skill, cand: SkillCandidate) -> Skill:
    ...
    try:
        system = (
            "You are EvoSkill's Skill Merger.\n"
...
def _merge_metadata(old: Dict, extra: Optional[Dict], cand: SkillCandidate) -> Dict:
    ...
    memory_kind = candidate_memory_kind(cand)
    out = merge_memory_metadata(old, memory_kind=memory_kind)
    if extra:
        out.update(extra)
    out = merge_memory_metadata(out, memory_kind=memory_kind)
    ...
    # Merge capabilities: union of old + new, deduplicated
    cand_caps = list((cand.metadata or {}).get("capabilities") or [])
    if cand_caps:
        existing_caps = list(out.get("capabilities") or [])
        merged_caps = list(dict.fromkeys(existing_caps + cand_caps))
        out["capabilities"] = merged_caps
...
def _candidate_to_raw(cand: SkillCandidate) -> Dict:
    ...
    return {
        ...
        "metadata": merge_memory_metadata(cand.metadata, memory_kind=candidate_memory_kind(cand)),
    }
...
def _skill_for_llm(skill: Skill) -> Dict:
    ...
    return {
        ...
        "version": skill.version,
        "metadata": merge_memory_metadata(skill.metadata, memory_kind=skill_memory_kind(skill)),
    }
