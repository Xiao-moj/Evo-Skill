...
from ...memory import MEMORY_KIND_SKILL, merge_memory_metadata, normalize_memory_kind
...
def upsert_skill_md_id(md: str, *, skill_id: str) -> str:
    ...
    This is used when importing external skills so they get a stable EvoSkill-assigned ID, while
...
def _render_frontmatter(
    *,
    ...
    for key in sorted(_FRONTMATTER_METADATA_KEYS):
        value = metadata.get(key)
        if value is None:
            continue
        if key == "memory_kind":
            normalized = normalize_memory_kind(value)
            if normalized != MEMORY_KIND_SKILL or "memory_kind" in metadata:
                lines.append(f"{key}: {_q(normalized)}")
            continue
        if isinstance(value, list):
            ...
            if not rendered:
                continue
            lines.append(f"{key}:")
            for item in rendered[:20]:
                lines.append(f"  - {_q(item)}")
            continue
        scalar = str(value).strip()
        if scalar:
            lines.append(f"{key}: {_q(scalar)}")
...
def load_agent_skill_dir(
    ...
    *,
    ...
    deterministic_id_salt: str = "evoskill-agent-skill-import-v1",
...
def _extract_extra_metadata(meta: Dict[str, object]) -> Dict[str, Any]:
    """Returns persistable non-core metadata from parsed frontmatter."""
    ...
    for key in _FRONTMATTER_METADATA_KEYS:
        ...
        if scalar:
            out[key] = scalar
    return merge_memory_metadata(out, memory_kind=out.get("memory_kind", MEMORY_KIND_SKILL))
